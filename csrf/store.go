...
func (cs *cookieStore) Get(r *http.Request) ([]byte, error) {
	...
	//fmt.Println("[(cs *cookieStore) Get] *http.Reques=" , r , " cs.name=" ,cs.name ," cookie=", cookie)
	//fmt.Println("[(cs *cookieStore) Get] err==" , err)
	if err != nil {
		return nil, err
	}
	...
	//fmt.Println("[(cs *cookieStore) Get] cs.name=", cs.name , " cookie.Value=", cookie.Value, "token= " ,token )
...
func (cs *cookieStore) Save(token []byte, w http.ResponseWriter) error {
	...
	//fmt.Println("[(cs *cookieStore) Save] cookie=" , cookie) ;
