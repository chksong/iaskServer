...
func (cs *csrf) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	//  资源文件过滤掉
	if len(r.URL.Path )>=7 && r.URL.Path[0:7] == "/assets" {
		cs.h.ServeHTTP(w, r)
		return
	}
	//fmt.Println("r.URL.Path[0:6]=" , r.URL.Path[0:7])
	...
	fmt.Println("[CSRF] cs.st.Get(r) realToken=", realToken)
	if err != nil || len(realToken) != tokenLength {
		...
		fmt.Println("[CSRF] cs.st.Save(realToken, w)==", realToken)
	...
	if !contains(safeMethods, r.Method) {
		...
		//fmt.Println("[CSRF] requestToken==", requestToken)
...
func unauthorizedHandler(w http.ResponseWriter, r *http.Request) {
	http.Error(w, fmt.Sprintf("%s - %s",
		...
		/*http.StatusText(http.StatusBadRequest), FailureReason(r)),
